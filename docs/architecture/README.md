...
- [001 — Deserialize does not restore DSP state (and Rust's did)](001-deserialize-does-not-restore-dsp-state.md) — **RESOLVED in 2.5.0** ([ADR-0008](../adr/0008-serde-carries-live-dsp-state.md)); kept for the history. `*_from_json_str` used to rebuild naad components from scalars instead of restoring their live state. Rust serialized that state; the port does not, and the source comments claiming a `#[serde(skip)]` mirror were describing an attribute that does not exist in `rust-old/`.
